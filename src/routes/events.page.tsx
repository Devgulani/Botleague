...
const faqs = [
  {
    q: "Who can register for BotLeague events?",
    a: "Any student or builder aged 10+ with a registered team can compete in age-appropriate categories.",
  },
  {
    q: "What is the entry fee?",
    a: "Most regional events are free to enter; nationals carry a small platform fee covering insurance and judging.",
  },
  {
    q: "How are events judged?",
    a: "Expert-led panels using standardized rubrics published in our public rulebook before each season.",
  },
  {
    q: "Are international events live-streamed?",
    a: "Yes — every national and international round streams on the BotLeague Network.",
  },
];
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {[
            ...
            <motion.div
              ...
              className="rounded-2xl border border-hairline bg-surface-1 p-6 text-center shadow-[0_24px_60px_-30px_rgba(255,45,45,0.4)] transition hover:border-accent/40"
            >
              <Trophy className="mx-auto h-8 w-8 text-accent" />
              <div className="mt-3 text-xs font-bold uppercase tracking-[0.25em] text-muted-foreground">
                {p.tier}
              </div>
...
function CardGrid({
  ...
  if (items.length === 0)
    return <p className="text-sm text-muted-foreground">No events match this filter.</p>;
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((e, i) => (
        ...
        >
          ...
          <button
            onClick={() => {}}
            className="mt-5 w-full rounded-md bg-accent py-2 text-xs font-bold tracking-wider text-accent-foreground transition hover:brightness-110"
          >
      ...
      <ol className="space-y-6">
        {steps.map((s, i) => (
          ...
          >
            ...
            <div
              className={`rounded-xl border border-hairline bg-surface-1 p-4 ${i % 2 ? "md:col-start-2 md:ltr" : ""}`}
            >
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="rounded-xl border border-hairline bg-surface-1 transition hover:border-accent/40">
      <button
        onClick={() => setOpen((o) => !o)}
      ...
      >
        {q}
        <ChevronDown
          className={`h-4 w-4 transition-transform duration-300 ${open ? "rotate-180 text-accent" : "text-muted-foreground"}`}
        />
      </button>
