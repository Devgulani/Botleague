...
function SignupPage() {
  ...
  const onGuest = () => { setGuestSession(); navigate({ to: "/" }); };
  ...
  return (
    <AuthShell
      ...
      footer={<>Already in? <Link to="/login" className="font-semibold text-accent">Log in</Link></>}
    >
      <form onSubmit={onSubmit} className="space-y-4">
        <Field label="Full Name"><input value={f.name} onChange={set("name")} className={inputCls} placeholder="Ada Lovelace" required /></Field>
        <Field label="Email"><input value={f.email} onChange={set("email")} type="email" className={inputCls} placeholder="you@arena.com" required /></Field>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Password"><input value={f.pwd} onChange={set("pwd")} type="password" className={inputCls} placeholder="••••••••" required /></Field>
          <Field label="Confirm"><input value={f.cpwd} onChange={set("cpwd")} type="password" className={inputCls} placeholder="••••••••" required /></Field>
        </div>
        <Field label="College / Organization"><input value={f.org} onChange={set("org")} className={inputCls} placeholder="IIT Bombay" /></Field>

        <label className="flex items-start gap-2 text-xs text-muted-foreground">
          <input type="checkbox" checked={f.terms} onChange={set("terms")} className="mt-0.5 accent-[--color-accent]" />
          <span>I agree to the <a className="text-accent">Terms</a> and <a className="text-accent">Privacy Policy</a>.</span>
        </label>
