...
function ForgotPage() {
  ...
  return (
    <AuthShell
      ...
      footer={<Link to="/login" className="inline-flex items-center gap-1 font-semibold text-accent"><ArrowLeft className="h-3 w-3" /> Back to login</Link>}
    >
      {!sent ? (
        <form
        ...
        >
          <label className="block">
            ...
            <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" required className={inputCls} placeholder="you@arena.com" />
          </label>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-md border border-cyan/40 bg-cyan/10 p-4 text-sm text-cyan">
