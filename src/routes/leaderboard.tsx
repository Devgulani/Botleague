...
type Row = { rank: number; team: string; college: string; points: number; wins: number; badges: number };
...
function LeaderboardPage() {
  ...
  return (
    <PageShell>
      <PageHero eyebrow="Rankings" title="National Leaderboard" subtitle="Live standings across every BotLeague event of the season." />

      <Section>
        <div className="grid gap-5 sm:grid-cols-3">
          {top3.map((r, i) => <PodiumCard key={r.team} row={r} place={i} />)}
        </div>
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-hairline">
          <div className="grid grid-cols-[60px_1.6fr_1.4fr_1fr_0.8fr_0.8fr] gap-2 border-b border-hairline bg-surface-2 px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
            <span>Rank</span><span>Team</span><span>College</span><span>Points</span><span>Wins</span><span>Badges</span>
          </div>
          {slice.map((r, i) => (
            <motion.div
              key={r.team}
          ...
          {slice.length === 0 && <div className="bg-surface-1 p-8 text-center text-sm text-muted-foreground">No teams match.</div>}
        </div>

        <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
          <span>Page {page + 1} of {pages}</span>
          <div className="flex gap-2">
            <button disabled={page === 0} onClick={() => setPage((p) => p - 1)} className="rounded border border-hairline bg-surface-1 px-3 py-1.5 font-semibold disabled:opacity-40">Prev</button>
            <button disabled={page >= pages - 1} onClick={() => setPage((p) => p + 1)} className="rounded border border-hairline bg-surface-1 px-3 py-1.5 font-semibold disabled:opacity-40">Next</button>
          </div>
        </div>
      </Section>
    </PageShell>
...
function PodiumCard({ row, place }: { row: Row; place: number }) {
  const accents = [
    { ring: "border-gold/60", glow: "shadow-[0_30px_80px_-30px_rgba(245,196,81,0.55)]", icon: Trophy, color: "text-gold", label: "1ST" },
    { ring: "border-cyan/50", glow: "shadow-[0_30px_80px_-30px_rgba(42,215,255,0.45)]", icon: Medal, color: "text-cyan", label: "2ND" },
    { ring: "border-accent/50", glow: "shadow-[0_30px_80px_-30px_rgba(255,45,45,0.45)]", icon: Award, color: "text-accent", label: "3RD" },
  ][place];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <span className={`text-xs font-bold uppercase tracking-[0.25em] ${accents.color}`}>{accents.label}</span>
      ...
      <div className="mt-5 font-display text-4xl font-bold tabular-nums">{row.points.toLocaleString()}</div>
