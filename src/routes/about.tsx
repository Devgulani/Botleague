    ...
    <PageShell>
      ...
      <Section>
        <div className="grid gap-6 md:grid-cols-2">
          <Card icon={Target} title="Mission" body="Create a structured, standardized, world-class competitive robotics ecosystem for every Indian builder." />
          <Card icon={Eye} title="Vision" body="A future where engineering talent is discovered in the arena and recruited into industry." />
        </div>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {stats.map((s, i) => (
              ...
              >
                ...
                <div className="mt-1 text-xs font-bold uppercase tracking-wider text-muted-foreground">{s.l}</div>
              </motion.div>
            ))}
          </div>
        </div>
            ...
            <ol className="space-y-6">
              {timeline.map((s, i) => (
                ...
                >
                  ...
                  <div className={`rounded-xl border border-hairline bg-surface-1 p-4 ${i % 2 ? "md:col-start-2" : ""}`}>
                    <div className="text-xs font-bold uppercase tracking-wider text-accent">{s.y}</div>
        ...
        <div className="mt-14 grid gap-5 md:grid-cols-2">
          <Card icon={BookOpen} title="Open Rulebook" body="Every category is judged by published rubrics — transparency by default." />
          <Card icon={Users} title="Community Impact" body="40+ student-run clubs activated; 200+ mentorships matched per season." />
        </div>
      </Section>
    </PageShell>
  );
