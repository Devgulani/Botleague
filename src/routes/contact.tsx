...
function ContactPage() {
  ...
  return (
    <PageShell>
      <PageHero eyebrow="Get In Touch" title="Talk To The League." subtitle="Press, partnerships, support — we read everything." />
      <Section>
        <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr]">
          <div className="space-y-4">
            <InfoCard icon={Mail} label="Email" value="hello@botleague.in" />
            <InfoCard icon={Phone} label="Phone" value="+91 80 4567 8901" />
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Name" error={errors.name}>
                <input value={form.name} onChange={set("name")} maxLength={80} className={input} placeholder="Ada Lovelace" />
              </Field>
              <Field label="Email" error={errors.email}>
                <input value={form.email} onChange={set("email")} maxLength={160} className={input} placeholder="you@arena.com" />
              </Field>
            </div>
            <div className="mt-4">
              <Field label="Subject" error={errors.subject}>
                <input value={form.subject} onChange={set("subject")} maxLength={120} className={input} placeholder="What's this about?" />
              </Field>
            </div>
            <div className="mt-4">
              <Field label="Message" error={errors.message}>
                <textarea value={form.message} onChange={set("message")} maxLength={1000} rows={6} className={`${input} resize-none`} placeholder="Tell us more..." />
              </Field>
            </div>
            <button
              type="submit"
            ...
            {sent && <p className="mt-3 text-xs text-cyan">Thanks — we'll reply within 48 hours.</p>}
          </motion.form>
        </div>
        ...
        <div className="mt-14">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {[
              ...
              <div key={q} className="rounded-xl border border-hairline bg-surface-1 p-4">
...
const input = "w-full rounded-md border border-hairline bg-surface-2 px-3 py-2.5 text-sm placeholder:text-muted-foreground/70 focus:border-accent/60 focus:outline-none";
...
function InfoCard({ icon: Icon, label, value }: { icon: typeof Mail; label: string; value: string }) {
  return (
    <div className="flex items-start gap-4 rounded-2xl border border-hairline bg-surface-1 p-5">
